...
#     model = nn.DataParallel(model).cuda()
    ...
    for ses in range(start_sess, args.num_task):
        ...
        # Create new loaders for testing (using the poisoned test dataset)
        _, _, _, test_loader, _ = inc_dataset.new_task(memory)

        # Evaluate on the poisoned test dataset
        acc_task_test = main_learner.meta_test(main_learner.best_model, memory, inc_dataset)

        # Save the test accuracy
        with open(args.savepoint + "/acc_task_test_" + str(args.sess) + ".pickle", 'wb') as handle:
            pickle.dump(acc_task_test, handle, protocol=pickle.HIGHEST_PROTOCOL)
